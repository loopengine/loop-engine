  ...
  return (
    <svg width="100%" viewBox={`0 0 ${width} 180`} className="rounded border bg-white">
      {definition.transitions.map((t, idx) => {
        const fromIdx = definition.states.findIndex((state) => state.id === t.from);
        const toIdx = definition.states.findIndex((state) => state.id === t.to);
        const x1 = 40 + fromIdx * gap;
        const x2 = 40 + toIdx * gap;
        const y = 110 + (idx % 2) * 12;
        const completed = completedTransitions.includes(t.id);
        return (
          <line
            key={`${String(t.id)}-${idx}`}
            x1={x1}
